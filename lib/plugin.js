...
export default <%= options.blockLoading ? 'async ' : ''%>(context, inject) => {
  const { app: { router, store }, $config } = context
  let options = <%= JSON.stringify(options) %>;
  if ($config.matomo) {
  	options = Object.assign(options, $config.matomo);
  }
  ...
  if (window.Piwik) {
    ...
    Object.defineProperty(window, 'Piwik', {
      ...
      set (newVal) {
        ...
        _windowPiwik = newVal
        _tracker = createTracker(delayedCalls, options)
...
function createTracker (delayedCalls = [], options) {
  if (!window.Piwik) {
    <% if(debug || options.debug) { %>
